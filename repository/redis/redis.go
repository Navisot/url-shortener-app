...
// NewRedisRepository returns a redis repo
...
// generateKey generates a redis key in order to save it
...
func (r *redisRepository) Find(code string) (*shortener.Redirect, error) {
	...
	if len(data) == 0 {
		return nil, errors.Wrap(shortener.ErrRedirectNotFound, "repository.Redirect.Find.NoResults")
	}
